...
def merge_image():
    ...
    try:
        ...
        if img_width == "원본유지":
            img_width = -1 # -1 일때는 원본 기준으로 이미지를 통합
        else:
        ...
        for idx, img in enumerate(images):
            # width 원본유지가 아닐때는 아닐때는 이미지 크기 조정
            if img_width > -1:
        ...
        # 포맷 옵션 처리
        file_name = "jeffkim_photo."+ img_format
        dest_path = os.path.join(txt_dest_path.get(), file_name) # 최종 출력 경로
...
def start():
    ...
    if len(txt_dest_path.get()) == 0:
        msgbox.showwarning("경고", "저장 경로가 설정되지 않았습니다.")
        return
...
# 진행 상황 Progress Bar
fram_progress = LabelFrame(root, text="진행상황")
fram_progress.pack(fill="x", padx=5, pady=5, ipady=5)

p_var = DoubleVar()
progress_bar = ttk.Progressbar(fram_progress, maximum=100, variable=p_var)
...
btn_close = Button(frame_run, padx=5, pady=5, text="닫기", width=15, command=root.quit)
btn_close.pack(side="right", padx=5, pady=5)

btn_start = Button(frame_run, padx=5 , pady=5, text="시작", width=15, command=start)
